...
const HOURS_BETWEEN_ALERTS = 6; // só repete alerta depois de 6 horas
...
async function checkAllCompetitions() {
  console.log([${new Date().toLocaleString()}] Checando partidas...);
  ...
  for (const comp of COMPETITIONS_TO_MONITOR) {
    ...
    if (
      ...
      console.log("Ja alertado recentemente (" + comp + "), ignorando...");
      continue;
    }
    ...
    const msg =
      "Alerta: " +
      SEQUENCE_LENGTH +
      " jogos seguidos com menos de 2 gols em " +
      comp +
      "!";
    console.log(msg);
